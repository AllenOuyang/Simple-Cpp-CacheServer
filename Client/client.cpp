...
int main(int argc, char** argv)
{
    ...
    // 2. 连接服务器IP port
    int ret = tcp.connectToHost("127.0.0.1", 25000);
    if (ret == -1)
    {
        return -1;
    }
    ...
    if(operation == "put")
        while (true)
        {
            ...
            std::cout << "send: " << "[k] = " << key << " " << "[v] = " << value << std::endl;
            // 发送数据
            tcp.sendMsg(std::to_string(key) + " " + std::to_string(value));
        ...
        while (true)
        {
            int key = rand() % 120;
            // 发送数据
            tcp.sendMsg(std::to_string(key));
            std::cout << "get: " << tcp.recvMsg() << std::endl;
